...
function isPalindrome(str) {
  ...
  for (let i = 0; i < cleanStr.length; i += 1) {
    if (
      cleanStr.charAt(i).toLocaleLowerCase() !==
      cleanStr.charAt(cleanStr.length - 1 - i).toLocaleLowerCase()
    ) {
      return false;
    }
  }
...
function extractNameFromTemplate(value) {
  const templateStart = 'Hello, '.length;
  return value.slice(templateStart, -1);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < letters.length; i += 1) {
    ...
    if (code >= 65 && code <= 77) {
      letters[i] = String.fromCodePoint(+code + 13);
...
function getCardId(value) {
  ...
  const weight = value.slice(0, -1);
  if (value.endsWith('♣')) {
    return cards.indexOf(weight);
  }
  if (value.endsWith('♦')) {
    return cards.indexOf(weight) + 13;
  }
  if (value.endsWith('♥')) {
    return cards.indexOf(weight) + 26;
  }
  return cards.indexOf(weight) + 39;
}
